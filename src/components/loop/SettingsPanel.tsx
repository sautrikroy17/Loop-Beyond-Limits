...
const PREMIUM_MODES: {
  ...
  },
  {
    id: "night-drive",
    label: "Night Drive",
    sub: "Deep indigo & slow sweeping motion",
    icon: <Moon />,
    color: "oklch(0.60 0.28 285)",
  },
  {
    id: "club-neon",
    label: "Club Neon",
    sub: "High contrast fast physics",
    icon: <Zap />,
    color: "oklch(0.85 0.35 320)",
  },
  {
    id: "dreamscape",
    label: "Dreamscape",
    sub: "Soft pastel washes & high blur",
    icon: <Sparkles />,
  ...
  },
  {
    id: "party",
    label: "Party Mode",
    sub: "Max neon visuals",
    icon: <Zap />,
    color: "oklch(0.85 0.34 290)",
  },
];
...
function SectionHeader({ icon, title }: { icon: React.ReactNode; title: string }) {
  return (
    <div className="mb-4 flex items-center gap-2.5">
      ...
      <span className="text-[10px] font-bold uppercase tracking-[0.38em] text-white/40">
        {title}
      </span>
    </div>
  );
...
export function SettingsPanel({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const timeRemainingStr = timeRemainingMs > 0 ? `${Math.ceil(timeRemainingMs / 60000)}m left` : "";
                ...
                <div className="grid grid-cols-2 gap-3 mt-4">
                  {PREMIUM_MODES.map((mode) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`p-2 rounded-full mb-3 ${isActive ? "bg-white/20" : "bg-white/5"}`}
                          style={{ color: mode.color }}
                        >
                          {mode.icon}
                        </div>
                        <div className="text-[13px] font-bold text-white mb-1">{mode.label}</div>
                        <div className="text-[10px] text-white/40 text-left leading-tight">
                          {mode.sub}
                        </div>
                      </button>
                    );
                    ...
                    {SLEEP_TIMER_OPTIONS.map((opt) => (
                      <button
                        ...
                        className={`px-4 py-2 rounded-lg text-[11px] font-bold uppercase tracking-wider transition-colors ${
                          (opt.id === 0 && !s.sleepTimerTarget) ||
                          (opt.id !== 0 &&
                            s.sleepTimerTarget !== null &&
                            timeRemainingMs > 0 &&
                            Math.abs(s.sleepTimerTarget - Date.now() - opt.id * 60000) < 600000)
                ...
                <div className="mb-6">
                  <div className="flex justify-between mb-2">
                    ...
                    <span className="text-[11px] font-mono text-white/40">
                      {Math.round(s.effectIntensity * 100)}%
                    </span>
                  </div>
                  <div className="relative h-6 flex items-center">
                    ...
                    />
                    <div
                      className={`h-1.5 w-full rounded-full bg-white/10 overflow-hidden ${s.premiumMode === "focus" || s.premiumMode === "party" ? "opacity-30" : ""}`}
                    >
                      <div
                ...
                <div className="flex items-center justify-between py-1">
                  <div>
                    ...
                    <div className="mt-0.5 text-[10px] text-white/40">
                      Force refresh if the UI gets stuck
                    </div>
                  </div>
                  <button
