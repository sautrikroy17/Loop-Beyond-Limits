            ...
            <div className="divide-y divide-white/5 p-2">
              {results.map((track) => (
                <div key={track.id} className="group flex items-center gap-3 rounded-xl p-2 hover:bg-white/5 transition-colors">
                  ...
                  <div className="min-w-0 flex-1" onClick={() => { playTrack(track); onClose(); }}>
                    <div className="truncate text-sm font-medium text-white cursor-pointer">{track.title}</div>
...
function LyricsPanel({ track }: { track: Track | null }) {
  ...
  useEffect(() => {
    if (!track) { setLines([]); setPlainLyrics(null); return; }
    setLoading(true);
    setLines([]);
    setPlainLyrics(null);
    getLyricsFn({ data: { title: track.title, artist: track.artist, duration: track.durationMs ? track.durationMs / 1000 : undefined } })
      .then((res) => {
        setLines(res.lines);
  ...
  if (lines.length > 0) {
    return (
      <div className="h-full overflow-y-auto px-2 py-8 space-y-4 scroll-smooth" style={{ scrollbarWidth: 'none' }}>
        {lines.map((line, i) => (
          <div
            key={i}
  ...
  if (plainLyrics) {
    return (
      <div className="h-full overflow-y-auto px-2 py-4 text-sm text-white/60 leading-relaxed whitespace-pre-line" style={{ scrollbarWidth: 'none' }}>
        {plainLyrics}
      </div>
    );
  }
...
function RecommendationsFeed({ currentTrack }: { currentTrack: Track | null }) {
  ...
  return (
    <div className="h-full overflow-y-auto" style={{ scrollbarWidth: 'none' }}>
      <div className="pb-4 text-[10px] uppercase tracking-widest text-white/30 px-1 pt-2">Up Next / Recommended</div>
      {loading && (
      ...
      <div className="space-y-1">
        {recs.map((track) => (
          <div key={track.id} className="group flex items-center gap-3 rounded-xl px-2 py-2 hover:bg-white/5 transition-colors">
...
function MusicSeekbar() {
  ...
  return (
    <div className="flex items-center gap-3 w-full">
      <span className="w-9 shrink-0 text-right text-[11px] tabular-nums text-white/40">{fmt(displayed)}</span>
      <div className="relative flex-1 group">
        ...
        <input
          ...
          step={0.5}
          value={displayed}
          onChange={(e) => { setIsDragging(true); setDragValue(Number(e.target.value)); }}
...
export function MusicPlayer() {
  const {
    currentTrack, isPlaying, volume, isShuffle, repeatMode, isLoadingTrack,
    togglePlayPause, nextTrack, prevTrack, toggleShuffle, toggleRepeat, setVolume,
  } = usePlayback();
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === ' ' && (e.target as HTMLElement).tagName !== 'INPUT') { e.preventDefault(); togglePlayPause(); }
    };
      ...
      <div className="flex w-full max-w-[420px] shrink-0 flex-col border-r border-white/8 bg-black/20 backdrop-blur-xl">
        ...
        <div className="shrink-0 border-t border-white/8 bg-black/30 px-6 py-5 space-y-4">
          ...
          <div className="flex items-center justify-between">
            ...
            {/* Prev */}
            <button onClick={prevTrack} className="rounded-full p-2 text-white/60 hover:text-white transition-colors">
            ...
            <button
              ...
              style={{ background: 'linear-gradient(135deg, oklch(0.75 0.22 290), oklch(0.72 0.2 240))', boxShadow: '0 0 40px -5px oklch(0.7 0.22 290 / 0.6)' }}
            >
              {isLoadingTrack
            ...
            {/* Next */}
            <button onClick={nextTrack} className="rounded-full p-2 text-white/60 hover:text-white transition-colors">
      ...
      <div className="flex flex-1 flex-col overflow-hidden bg-black/10 backdrop-blur-xl">
        ...
        <div className="flex shrink-0 items-center gap-1 border-b border-white/8 px-4 py-3">
          ...
          >
            ...
            <kbd className="ml-2 hidden rounded bg-white/10 px-1.5 py-0.5 text-[10px] sm:inline-block">⌘K</kbd>
          </button>
