...
function ReactiveAlbumArt({ src, trackId }: { src?: string; trackId?: string }) {
  ...
  useEffect(() => {
    ...
    return subscribeToAudio((data) => {
      ...
      const glowAlpha = (0.12 + data.bass * 0.5).toFixed(3);
  ...
  if (queue.length === 0) {
    return (
      <EmptyState
        ...
        text="Queue is empty — autoplay will continue"
      />
    );
  }
  return (
    <div className="h-full overflow-y-auto px-3 py-3 space-y-1" style={{ scrollbarWidth: "none" }}>
      {queue.map((track, i) => (
        <div
          key={`${track.id}-${i}`}
          className="group flex items-center gap-3 rounded-xl px-2 py-2.5 hover:bg-white/5 transition-colors"
        >
  ...
  return (
    ...
    >
      ...
      <form
        ...
        className="border-b border-white/[0.06] p-2 flex items-center gap-2"
      >
        <input
          type="text"
      ...
      {playlists.length === 0 ? (
        <div className="px-3 py-5 text-center text-[11px] text-white/30">No playlists yet</div>
      ) : (
      ...
      {isOpen && (
        ...
        >
          ...
          <div className="relative z-10 flex shrink-0 items-center justify-between px-6 py-4">
            <p className="text-[9px] font-medium uppercase tracking-[0.4em] text-white/28">
              Now Playing
            </p>
          ...
          <div className="relative z-10 flex flex-1 flex-col md:flex-row overflow-y-auto md:overflow-hidden pb-[env(safe-area-inset-bottom)]">
            ...
            <div className="flex w-full md:max-w-[22rem] shrink-0 flex-col justify-center gap-5 px-6 md:px-8 py-4">
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  <ReactiveAlbumArt src={currentTrack?.albumArt} trackId={currentTrack?.id} />
                </motion.div>
              </AnimatePresence>
                ...
                {(["lyrics", "queue"] as Tab[]).map((t) => (
                  <button
                    ...
                    className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium capitalize transition-colors ${
                      tab === t ? "bg-white/[0.08] text-white" : "text-white/30 hover:text-white/65"
                    }`}
                  >
                    {t === "lyrics" ? (
