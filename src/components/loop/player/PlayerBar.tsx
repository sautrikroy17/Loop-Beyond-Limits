 ...
 * - Click/drag to seek
 */
function EQProgressCanvas({ pct, onSeek }: { pct: number; onSeek: (pct: number) => void }) {
...
export function PlayerBar({
  ...
  const handleEQSeek = useCallback(
    (p: number) => {
      if (!duration) return;
                ...
                <button
                  onClick={() => {
                    ...
                    if (isLiked(currentTrack.id)) {
                      ...
                      useListeningIntelligence.getState().markLiked(currentTrack.id);
                    }
                  }}
                  title="Like"
                  className={`hidden md:flex h-8 w-8 shrink-0 items-center justify-center rounded-full transition-all hover:bg-white/[0.07] ${
