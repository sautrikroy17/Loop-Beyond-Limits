...
function AlbumRow({ album, onPlay }: { album: any; onPlay: () => void }) {
  return (
    <div
      onClick={onPlay}
...
function PlaylistRow({ playlist, onPlay }: { playlist: any; onPlay: () => void }) {
  return (
    <div
      onClick={onPlay}
      className="group flex cursor-pointer items-center gap-3 rounded-xl p-2.5 transition-colors hover:bg-white/[0.04]"
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="truncate text-[11px] text-white/40">
          Playlist • {playlist.artist || "Curated"}
        </div>
      </div>
    </div>
  );
...
export function SearchModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const [recentSearches, setRecentSearches] = useState<string[]>(() => {
    try {
      return JSON.parse(localStorage.getItem("loop-recent-searches") ?? "[]");
    } catch {
      return [];
    }
  });
          ...
          >
            ...
            <div className="flex items-center gap-3 border-b border-white/[0.06] px-5 py-4">
              ...
              {isSearching ? (
                ...
                <kbd className="hidden shrink-0 rounded border border-white/[0.08] bg-white/[0.04] px-1.5 py-0.5 text-[10px] text-white/28 sm:block">
                  ESC
                </kbd>
              )}
            </div>
                ...
                {(["all", "songs", "albums", "playlists", "artists"] as SearchTab[]).map((t) => (
                  <button
                    ...
                    className={`rounded-lg px-3 py-1.5 text-xs font-medium capitalize transition-colors ${
                      tab === t ? "bg-white/[0.08] text-white" : "text-white/32 hover:text-white/70"
                    }`}
                  >
                    {t}
            ...
            <div className="max-h-[60vh] overflow-y-auto" style={{ scrollbarWidth: "none" }}>
                ...
                <div className="px-4 py-3">
                  <div className="mb-3 flex items-center justify-between text-[10px] font-medium uppercase tracking-[0.35em] text-white/22">
                    ...
                    <button
                      onClick={() => {
                        setRecentSearches([]);
                        localStorage.removeItem("loop-recent-searches");
                      }}
              ...
              {query.length < 2 && !showRecent && (
                <div className="px-4 py-5 text-center">
                  <p className="mb-4 text-[12px] text-white/22">
              ...
              {results.playlists.length > 0 && (tab === "all" || tab === "playlists") && (
                <div className="p-2">
                  ...
                  <div className="grid grid-cols-2 gap-2 sm:grid-cols-2">
                    {(tab === "all" ? results.playlists.slice(0, 4) : results.playlists).map(
                      (playlist) => (
                        <PlaylistRow
                          key={playlist.id}
                          playlist={playlist}
