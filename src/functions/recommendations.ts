...
import {
  ...
  getPlaylistDetails,
  searchPlaylist,
} from "../server/services/youtubeMusic";
...
const GARBAGE_REGEX =
  /workout|karaoke|cover|tribute|compilation|80s|90s|lofi hip hop radio|sex playlist|vocal version|instrumental cover|8d audio|slowed \+ reverb|bass boosted|tiktok version|unofficial|remake|live at|acoustic cover/i;
...
function generateMicroLabel(genre: string, tasteIdentity: string, isBasedOn: boolean): string {
  ...
  if (genre.toLowerCase().includes("bollywood") || genre.toLowerCase().includes("hindi")) {
    labels.push(
      "Shared cinematic vocals",
      "Similar emotional intensity",
      "Fans of Arijit love this",
    );
  } else if (genre.toLowerCase().includes("r&b") || genre.toLowerCase().includes("dark")) {
    labels.push(
      "Same late-night energy",
      "Shared dark synth textures",
      "Fans of After Hours love this",
      "Cinematic electronic crossover",
    );
  } else if (genre.toLowerCase().includes("trap") || genre.toLowerCase().includes("hip hop")) {
    labels.push(
      "Similar heavy bassline",
      "Underground club energy",
      "Shared atmospheric production",
    );
  } else if (genre.toLowerCase().includes("pop")) {
    labels.push(
      "Main character energy",
      "Similar melodic structure",
      "Glossy cinematic production",
    );
  } else {
...
export const getDiscoverySectionsFn = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<DiscoverySection[]> => {
    ...
    const selectedCharts = CHART_POOL[g1] || CHART_POOL["Pop"] || [];
...
export const getAlbumDetailsFn = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<DiscoveryTrack[]> => {
    ...
    return tracks.map(toTrack).map((t) => ({
      ...t,
      artist:
        !t.artist || t.artist === "Unknown Artist" || t.artist === "Unknown"
